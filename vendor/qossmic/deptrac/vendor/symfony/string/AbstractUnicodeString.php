...
use DEPTRAC_INTERNAL\Symfony\Component\String\Exception\ExceptionInterface;
use DEPTRAC_INTERNAL\Symfony\Component\String\Exception\InvalidArgumentException;
use DEPTRAC_INTERNAL\Symfony\Component\String\Exception\RuntimeException;
/**
...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    {
        $str = clone $this;
        if (!$compat || !\defined('DEPTRAC_INTERNAL\\Normalizer::NFKC_CF')) {
    ...
    {
        $str = $this->camel();
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'DEPTRAC_INTERNAL\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
