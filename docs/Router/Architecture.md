...
The Avax HTTP Router is a production-ready, type-safe HTTP request router designed for modern PHP applications. It provides a clean separation between route registration (DSL), runtime resolution, and execution pipeline.
...
*This document defines the canonical architecture. Implementation details may vary but the public contracts and behaviors documented here are guaranteed stable.*