...
Technically, `Observe/Inspect` contains utilities that read container state, prototypes, and resolution metadata to
produce human-facing diagnostics (CLI dumps, inspectors, managers). These tools are about understanding the container,
...
Inspectors typically consume prototype models, definition store data, and telemetry timelines. They format those into
readable reports for developers (often via CLI commands).
