...
Think of it like a flight plan printout. The prototype is the plan; this dumper prints it in a structured way you can
skim quickly.
...
You run a `container:inspect` command and it shows a service is not instantiable and has a constructor parameter with an
unknown type. Without this dumper you’d need to dig into prototype internals. With it, you immediately see the
constructor signature and the list of injections and you fix the missing binding.
...
`dump()` builds an array of lines, formatting the constructor as a `MethodPrototype` when available, then iterates
`injectedProperties` and `injectedMethods` and formats each. Helpers `formatMethod()`, `formatParameter()`, and
`formatProperty()` produce compact signatures with type hints and default values.
...
- Why this file lives in `Observe/Inspect`: it’s not part of resolution; it’s part of observability and developer
  tooling.
