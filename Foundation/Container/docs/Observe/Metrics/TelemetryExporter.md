...
Think of it like shipping labels. The container prepares the package (metric name + value); the exporter decides which
courier to use (backend).
...
You start with a log-based exporter for development. Later you move to a full monitoring stack. You keep the same
container code and swap out the `TelemetryExporter` implementation.
...
The interface defines two operations: `increment()` for counter metrics and `observe()` for numeric observations.
Implementations can batch, buffer, or send immediately; they can also apply naming conventions and labels externally.
