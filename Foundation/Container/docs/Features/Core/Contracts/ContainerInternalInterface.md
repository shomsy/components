...
## Terminology (MANDATORY, EXPANSIVE)- **Internal contract

**: API intended for container internals, not typical application code.
...
A property injector wants to resolve a dependency while preserving the current resolution chain. If the container
supports internal context resolution, it calls `resolveContext($context->child(...))` instead of `get()`.
...
Implementations expose underlying stores and provide an entry point for context-aware resolution that bypasses “fresh
context” behavior.
