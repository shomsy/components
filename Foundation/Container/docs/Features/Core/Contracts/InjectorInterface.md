...
A legacy object is created outside the container. The app calls `injectInto($object)` to fill `#[Inject]` properties.
Before doing it, the app calls `canInject($object)` to confirm the container can resolve everything.
...
Implementations typically scan targets via reflection, build prototypes, resolve dependencies, and apply injection
safely.
...
You use this when you want the injector to “know” some options up front, so you don’t have to pass the same knobs on
every call.
