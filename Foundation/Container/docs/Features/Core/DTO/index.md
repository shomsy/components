...
Small immutable data transfer objects used to communicate outcomes and diagnostics in a structured way. They exist to
replace ad-hoc arrays and inconsistent return shapes.
...
Guards and validators often return `ErrorDTO`/`SuccessDTO`. Injection tooling can produce `InjectionReport`. Exceptions
are thrown for hard failures; DTOs are used when you want structured, non-exception outcomes.
