...
A guard policy check fails. Instead of throwing immediately, the guard returns an `ErrorDTO`. A kernel step then decides
to throw a `ContainerException` with a safe message.
...
Used by guard/validation subsystems to communicate failures in a structured way. Can be converted into exceptions by
kernel steps.
