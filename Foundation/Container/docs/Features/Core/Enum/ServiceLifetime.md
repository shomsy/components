...
Like choosing whether to share a notebook: one notebook for the whole team (singleton), one per meeting (scoped), or one
per person every time (transient).
...
A database connection is singleton, a request context is scoped, and a lightweight formatter is transient. The container
uses these values to store or rebuild each type appropriately.
