...
## Terminology (MANDATORY, EXPANSIVE)- **PSR-11**: Standard interface for containers; defines exception interfaces.
...
A service cannot be resolved due to missing dependencies. The container wraps the failure as a `ContainerException` so
callers can catch a consistent type.
