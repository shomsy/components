...
A required property injection fails because no service is registered for a type. The injector throws
`ResolutionException` so the caller knows resolution failed.
