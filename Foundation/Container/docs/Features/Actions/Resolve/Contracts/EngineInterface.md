...
**: Component responsible for producing a service instance/value.
...
Like a vending machine interface: you insert a request (context) and get an item (resolved value). The interface defines
the buttons, not the machine internals.
...
You replace the engine implementation in tests with a fake that returns known values. Because the kernel depends on
`EngineInterface`, no pipeline code changes.
...
- `ContainerException` if called more than once (implementations should guard against double-initialization).
...
Not calling it before resolution; calling it twice and triggering initialization guards.

### Method: resolve(KernelContext $context, ?TraceObserverInterface $traceObserver = null): mixed

#### Technical Explanation (resolve)

Resolves the service described by the context, optionally emitting a trace to an observer.
...
- `TraceObserverInterface|null $traceObserver`
...
Ignoring overrides in the context; forgetting to pass a trace observer when debugging.
