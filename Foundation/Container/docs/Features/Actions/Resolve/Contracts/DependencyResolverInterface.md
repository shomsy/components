...
## Terminology (MANDATORY, EXPANSIVE)- **Parameter list**: Prototypes describing what arguments are needed.
...
Parameters describe needs, overrides supply manual values, container supplies services, context keeps the resolution
stack.

## Think of It

Like a recipe assistant: it reads the ingredient list (parameters), uses what you already have (overrides), and fetches
the rest from the pantry (container).
...
`InvocationExecutor` needs to call a callable. It asks the dependency resolver to turn parameter prototypes into actual
argument values. Swapping resolver implementations changes resolution behavior without touching invocation code.
...
Declares
`resolveParameters(array $parameters, array $overrides, ContainerInterface $container, ?KernelContext $context): array`.
...
##### For Humans: What This Means (resolveParameters)
...
##### Returns (resolveParameters)

- `array`: Ordered arguments.

##### Throws (resolveParameters)

- Implementation-specific resolution exceptions.

##### When to Use It (resolveParameters)

Called wherever argument lists are needed.

##### Common Mistakes (resolveParameters)
