...
Technically, `Features/Define/Store` holds the data structures that represent service definitions and the indices that
make them queryable (by id, by tag, by consumer context, etc.). Builders write into this folder’s models; runtime
resolution reads from them. The store is the authoritative source of truth for service metadata.

### For Humans: What This Means (Represent)

This is the container’s notebook. Builders write down facts here, and the runtime reads those facts later when it needs
to build objects.
...
`ServiceDefinition` is the per-service blueprint. `DefinitionStore` stores many definitions and keeps extra lookup
tables (tags, contextual overrides, extenders). Compiler passes can be applied to the store to pre-process definitions (
validation, normalization, optimization) before runtime resolution consumes them.
