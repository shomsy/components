...
- It removes the complexity of “where do I hook into definition processing?” by offering one consistent entry point: `process()`.
...
## Think of It
Think of a compiler pass like a spell-check pass on a document: it reads the whole thing and either fixes issues or complains.

### For Humans: What This Means (Think)
You catch mistakes early, before they turn into runtime crashes.

## Story Example
You want to enforce that every singleton service has a tag `singleton`. You write a compiler pass that scans the store and adds that tag automatically. Or you write a pass that throws if it finds bindings missing concrete implementations in production.
...
This interface defines one method: `process(DefinitionStore $definitions): void`. The container or bootstrapper is responsible for ordering and executing passes. The pass can mutate the store, because the store is the mutable registry for definitions.
