...
Your bootstrap code reads a profile, creates a cache manager and logger factory, and wants to pass them around with
config. Instead of passing three parameters everywhere, it passes one `ContainerWithInfrastructure`.
...
This is a `final readonly` DTO storing `ContainerConfig $config` and two `mixed` infrastructure references. It has only
a constructor.
