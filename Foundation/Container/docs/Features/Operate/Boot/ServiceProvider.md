...
- It removes the complexity of “where do I put container wiring?” by giving you two explicit hooks: `register()` and
  `boot()`.

### For Humans: What This Means (Summary)

It’s the standard template for “installing” a feature into your container. First you declare bindings, then you
optionally turn things on.
...
You create `LoggingServiceProvider`. In `register()`, you bind `LoggerFactory` and `LoggerInterface`. In `boot()`, you
initialize a global error handler and write a “logging ready” message. Your bootstrap code registers providers first,
then boots them as a group.
...
The base class stores an `Application` reference. The default implementations of `register()` and `boot()` are
intentionally empty, making them optional overrides. The class-level contract communicates a lifecycle rule: avoid
resolution in `register()`.
