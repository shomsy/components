...
Think of it like a weather report provider. The app asks “what’s the weather?” and doesn’t care if it came from a
website or a sensor.
...
In local dev you load `.env` file and return an array. In production you load from environment variables or a config
server. Both implement `EnvironmentLoaderInterface`, so bootstrap code stays the same.
...
The interface defines `loadEnvironment(): array`. Implementations should validate and throw a `RuntimeException` when
config is missing/invalid, or provide sensible defaults for optional keys (depending on policy).
