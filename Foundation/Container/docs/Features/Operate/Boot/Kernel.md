...
Think of it like a shopping list reader. If the list is missing, it doesn’t panic—it just says “okay, we’ll cook without
extras.”
...
In development, you forget to set `kernel.middlewares` in config. Without safety, your app could crash before routing.
With this kernel, middleware resolves to an empty array, and the rest of the lifecycle continues.
...
The constructor accepts an optional callable config resolver. If absent, it builds a default resolver that reads
`kernel.middlewares` via a `config()` helper. The resolver is stored as a `Closure` for consistent invocation.
Resolution catches all `Throwable` and enforces that the result is an array.
...
#### Technical Explanation (resolveConfiguredMiddlewares)

Invokes the config resolver and returns the middleware list, or `[]` on any error.

##### For Humans: What This Means (resolveConfiguredMiddlewares)

It answers: “what middleware should we apply?”

##### Parameters (resolveConfiguredMiddlewares)

- None.

##### Returns (resolveConfiguredMiddlewares)

- `array`

##### Throws (resolveConfiguredMiddlewares)

- No (it catches `Throwable`).

##### When to Use It (resolveConfiguredMiddlewares)

- When building the middleware pipeline for a request.

##### Common Mistakes (resolveConfiguredMiddlewares)
