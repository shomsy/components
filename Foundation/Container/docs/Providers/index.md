...
Technically, providers are boot-time registration modules: each provider is responsible for adding a set of
bindings/definitions into the container for a particular subsystem (HTTP, auth, database, filesystem, logging, etc.).
They keep integration wiring isolated and composable.

### For Humans: What This Means (Represent)

Providers are like plug-in installers. You enable one, and the container learns how to build and wire a whole feature
area.
...
Bootstrapping code selects and runs providers. Providers call registration APIs (bind/singleton/scoped, contextual
bindings, extenders) to populate `DefinitionStore`. The runtime engine then resolves services using those definitions.
