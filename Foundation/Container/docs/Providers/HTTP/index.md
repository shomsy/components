...
Technically, these providers register HTTP clients, router and middleware services, request/session scope helpers, and view rendering integrations. They often define scope boundaries (per-request) and register the services needed to build an HTTP pipeline.
...
HTTP providers register the services needed for request handling. The container then resolves those services per request, often using scoped lifetimes so request-specific state doesn’t leak.
