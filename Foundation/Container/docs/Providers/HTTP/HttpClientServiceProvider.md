...
Think of it like calling a friend with bad reception. The retry middleware is you saying “I’ll call again if the line
drops”.
...
Your payment service calls an external gateway. Instead of creating an HTTP client each time, it asks the container for
`HttpClient`. This provider makes sure the client exists and uses retry logic with logging.
...
`register()` binds `RetryMiddleware` using a closure that resolves `LoggerInterface`. It then binds `HttpClient` using a
closure that resolves the middleware and logger.
