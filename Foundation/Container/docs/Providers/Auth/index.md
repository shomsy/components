...
Technically, these providers register bindings for authentication helpers, security policies, and any per-request/scoped
auth state required by your application infrastructure. They keep auth wiring isolated so you can enable/disable or swap
...
Auth providers register their bindings into the container during boot. Other providers and runtime components then rely
on those bindings for request handling and security enforcement.
