...
Think of it like setting up a shared “water pump” for a building. You configure it once, and everyone uses the same
reliable system.
...
Your repository services need database access. Instead of every repository creating its own connection manager, they
typehint `ConnectionManager` (or ask for `'db'`). This provider ensures the container can supply it based on config.
...
`register()` binds `EventBus` as a singleton. It then binds `ConnectionManager` as a singleton via a closure that reads
config and resolves the event bus if available. Finally, it binds `'db'` as an alias to `ConnectionManager`.
