...
Technically, these providers register the “platform services” that many other subsystems depend on. By keeping them
separate, you can bootstrap the container in layers: core infrastructure first, then higher-level features.
...
Core providers usually run early and register base services. Domain providers then assume those services exist (config,
logger, filesystem) and build on top of them.
