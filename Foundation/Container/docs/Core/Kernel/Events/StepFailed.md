...
Event fired when a kernel pipeline step throws an exception, carrying timing, duration, step class, service ID, the
exception, and optional trace ID. It exists to drive error telemetry and debugging.
...
On failure, telemetry uses `StepFailed` to log the exception, duration, and service/trace for alerts and debugging
dashboards.
...
Immutable value object with readonly properties for step class, startedAt, endedAt, duration, serviceId, exception,
traceId set via constructor.
