...
Event fired when a kernel pipeline step begins execution, carrying step class, timestamp, service ID, and optional trace
ID. It exists to mark the start of step processing for telemetry and tracing.

### For Humans: What This Means (Summary)

It’s the “step started” message with when/what/for which service and optional trace.

## Terminology (MANDATORY, EXPANSIVE)- **Step class**: The fully qualified class name of the step starting.
...
Telemetry wants to measure step durations. On `StepStarted`, it records the start time for the step/service/trace, later
matching it with `StepSucceeded` or `StepFailed`.
...
Part of kernel event payloads consumed by telemetry/logging. Emitted by pipeline runners/steps when step execution
starts.
