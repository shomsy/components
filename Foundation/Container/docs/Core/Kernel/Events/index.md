...
Kernel step lifecycle events emitted during the resolution pipeline: start, success, and failure notifications. They
exist to provide structured telemetry payloads.
...
`StepStarted`, `StepSucceeded`, and `StepFailed` carry context for telemetry and debugging; telemetry collectors consume
them, and pipeline steps/runner emit them.
