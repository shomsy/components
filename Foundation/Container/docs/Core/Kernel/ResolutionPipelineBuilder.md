...
## Terminology (MANDATORY, EXPANSIVE)**Pipeline Assembly**: The process of creating and configuring resolution steps in the correct sequence. In this file, the defaultFromConfig() method performs this assembly. It matters because step order affects resolution correctness and performance.
...
This builder is the place where “your configuration becomes a working engine”. If something is miswired (wrong step order, missing collaborator), the failure will show up here or right after. Treat it like a pre-flight checklist: validate configs early, and test a couple of realistic profiles so you don’t discover wiring problems in production.
