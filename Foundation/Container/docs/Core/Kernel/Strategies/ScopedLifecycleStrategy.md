...
Caches service instances within a scope using `ScopeManager`, clearing them when the scope ends. It exists to implement
...
Like a hotel room key: valid for your stay (scope), discarded at checkout. ScopeManager is the front desk storing room
assignments.
...
In a web app, each request starts a scope. Services marked scoped are stored via `setScoped`; subsequent resolutions
reuse them. At request end, `endScope()` clears them.
