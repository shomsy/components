...
Think of it like a compiler flag that turns warnings into errors. You might be more annoyed at first, but you stop
...
In development, you want permissive behavior while prototyping. In production, you want strict behavior so misconfigured
services fail immediately and loudly. You enable strict policy, and `StrictResolutionPolicy` denies resolving unknown
classes, forcing you to fix the config instead of silently getting weird runtime behavior.
...
The policy reads a `ContainerPolicy` instance (injected), checks `$policy->strict`, and if strict mode is enabled it
denies resolution when `class_exists($abstract)` is false. Otherwise it allows resolution.
...
- Risk: Over-strict checks can block valid resolution patterns.
    ...
    - Recommended practice: if your container resolves interfaces, extend the policy to allow `interface_exists()` too (
      or document your conventions clearly).
