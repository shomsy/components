...
- This file defines an attribute-based rule that validates service class names (existence, abstract/interface
  allowances, required interfaces, forbidden classes).
...
Think of it like hiring rules: the applicant must exist, must not be “in training only” (abstract), must have required
certifications (interfaces), and must not be on the banned list.
...
You refactor a namespace and forget to update the service definition. In production, the container would crash later
when it tries to instantiate the missing class. With `ServiceValidationRule`, validation fails immediately during
registration with a clear “class doesn’t exist” error and you fix it before deployment.
...
The rule reads its constructor configuration (`allowAbstract`, `allowInterface`, `requiredInterfaces`,
`forbiddenClasses`). `validate()` verifies the input is a non-empty string, checks class/interface existence, optionally
denies interfaces, denies forbidden classes, checks abstract instantiability via reflection, and verifies required
interface implementation. `getErrorMessage()` describes constraints in human-readable form.
