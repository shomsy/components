...
Resolves options (output file, force flag, cache dir), checks for existing output, ensures directories exist and are writable, invokes `CompiledPrototypeDumper` to produce code, writes the compiled file, adjusts permissions, and reports size and success.
