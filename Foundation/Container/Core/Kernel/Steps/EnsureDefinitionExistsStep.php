...
final readonly class EnsureDefinitionExistsStep implements KernelStep
{
    ...
    public function __invoke(KernelContext $context): void
    {
        ...
        if ($this->definitions->has(abstract: $context->serviceId)) {
            ...
            $context->setMeta('definition', 'source', 'store');
            $context->setMeta('definition', 'instance', $this->definitions->get(abstract: $context->serviceId));
            return;
        }

        // Definition does not exist
        if ($this->autoDefine && $this->isAutoDefinable($context->serviceId)) {
            ...
            $context->setMeta('definition', 'source', 'auto');
            $context->setMeta('definition', 'instance', $definition);
        } else {
            $context->setMeta('definition', 'source', 'missing');
            $context->setMeta('definition', 'warning', "Missing definition for [{$context->serviceId}]. Resolution may fail if not literal.");

            if ($this->strictMode) {
                throw new \Avax\Container\Features\Core\Exceptions\ResolutionException(
