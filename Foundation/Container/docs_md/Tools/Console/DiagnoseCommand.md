...
Builds a diagnostics inspector from the container, gathers stats (definitions, instances, contextual bindings), and renders a console report with timeline and memory data from `DiagnosticsDashboard` and `metrics()`. Output is plain text with sections.
