...
## Quick Summary
- This file defines an attribute-based validation rule that checks whether dependency arrays in service definitions are well-formed and allowed.
...
## Story Example
Your system allows developers to register services via configuration. One config accidentally declares a dependency `"databse"` instead of `"database"`. Without validation, resolution fails later in a confusing way. With `DependencyValidationRule`, the registration step fails immediately with a clear error message and you fix it quickly.
...
The rule stores optional constraints (allowed services list, self-reference allowance flags, depth limit). `validate()` ensures the input is an array, then checks each entry for being a non-empty string, optionally checks whitelist membership, and validates format via regex. `getErrorMessage()` builds a human-readable message describing the active constraints.
...
- System-level reasoning: if you allow bad dependency declarations into the system, everything downstream becomes harder and noisier.
...
- `docs_md/Features/Define/Store/ServiceDependency.md`: The entity representing dependency relationships that this rule helps protect.
