...
## Story Example
Your form middleware needs a CSRF token manager. Instead of building it manually and figuring out storage/logging, it asks the container for `CsrfTokenManager`. This provider ensures it exists and is constructed with the correct dependencies.
...
`register()` binds `CsrfTokenManager` to a closure that constructs it using `Session` and `LoggerInterface` resolved from the container.
