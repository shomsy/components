...
## Story Example
Your CSRF token manager needs a session to store tokens. The security provider requests `Session` from the container. This provider ensures it exists and has an alias `'session'` for convenience.
...
`register()` binds `Session` as a singleton via a closure (placeholder for injecting configuration). It binds `SessionAdapter` to its class and binds `'session'` as an alias to the `Session` singleton.
