...
Technically, `Features/Operate/Config` holds configuration DTOs and profiles that control operational settings: cache directories, diagnostics flags, bootstrap profiles, and potentially infrastructure integrations. It’s distinct from “definitions” because it configures the container’s runtime machinery, not individual service bindings.
...
Boot code loads/creates config objects from environment sources. Other runtime subsystems (prototype cache, telemetry, scope) read those configs to decide where to store data and how strict to be.
