...
It stops the container from going too deep and crashing your process by throwing a clear error when dependency chains get out of control.

## Terminology
- **Resolution depth**: How deep the container is in nested dependency resolution.
...
### For Humans: What This Means
Depth is “how many dependencies deep you are.” Max depth is the safety line. Path shows the chain. Stack overflow is the crash this prevents.

## Think of It
Like a circuit breaker: it doesn’t prevent you from using electricity, but it stops the system before overheating burns the house down.

### For Humans: What This Means
It’s a safety cutoff that protects your app from extreme dependency recursion.

## Story Example
A complex service graph grows over time. One day, a bug creates a long chain of nested resolutions. Instead of crashing with a fatal error, DepthGuardStep throws a `ResolutionException` showing the path so the developer can fix the wiring.
...
- “This detects circular dependencies.” It doesn’t; it prevents excessively deep chains (circles are handled by a separate step).

### For Humans: What This Means
It’s about “too deep,” not “loops.” It’s a backup safety net.

## How It Works (Technical)
`__invoke` checks whether `KernelContext::$depth` is greater than the configured `maxDepth`. On overflow, it throws `ResolutionException` containing the max depth and the path.

### For Humans: What This Means
If the container goes past the configured depth, it stops immediately with a descriptive error.

## Architecture Role
Early guard step that protects the pipeline from pathological dependency graphs. Depends only on context depth/path and the `ResolutionException` type.
...
- **Practice: Fix roots**. Treat depth errors as a design smell—refactor dependency graphs rather than just increasing limits.

### For Humans: What This Means
If you hit this, your dependency graph is probably too complex. Fix the graph first; only raise the limit if you truly must.
